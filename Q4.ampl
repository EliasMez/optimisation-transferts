...
# tableau a double entrée dont la valeur correspond au nombre de fois ou un cadeau est acheté par un participant
param acheteur {CADEAUX, PARTICIPANTS} >=0;

# tableau a double entrée dont la valeur correspond au nombre de fois ou un cadeau est acheté de la part d'un participant
param deLaPart {CADEAUX, PARTICIPANTS} >=0;
...
# représente le prix qu'un participant doit payer au final
param montantParticipation {p1 in PARTICIPANTS} := sum {c in CADEAUX} ((prixCadeau[c] / sum {p in PARTICIPANTS} deLaPart[c,p]) * deLaPart[c,p1]); 
...
subject to equiteMin {p in PARTICIPANTS} :
	avance [p] + paye [p] - recu [p] >= trunc(montantParticipation[p]);

subject to equiteMax {p in PARTICIPANTS} :
	avance [p] + paye [p] - recu [p] <= trunc(montantParticipation[p]) + 1;
...
param deLaPart :
                                Claqueur Beleuleu Avare Depensier Farceur Econome :=
ordinateur                      1        0        0     1         0       0
tarte                           0        1        0     0         0       0
vase                            0        0        1     0         0       1
voyage                          0        1        0     1         0       0
plante                          0        1        0     1         0       1
bobRicard                       0        0        1     0         0       1
collier                         0        0        1     0         0       1
Beethoven                       1        1        1     1         1       1;
